        ...
        <aside className="w-64 bg-[#0f172a] text-white flex flex-col fixed inset-y-0 left-0 z-50">
            ...
            <div className="p-4 border-t border-white/5 bg-[#0a101f]">
                <div className="flex items-center gap-3 mb-4 px-2 py-2 rounded-xl bg-white/5">
                    <div className="w-9 h-9 rounded-full bg-gradient-to-br from-sky-400 to-blue-600 flex items-center justify-center text-white text-sm font-bold shadow-inner">
                        {(currentUser?.displayName || currentUser?.email || 'A')[0].toUpperCase()}
                    </div>
                    <div className="flex-1 min-w-0">
                        <p className="text-sm font-semibold truncate text-gray-100">{currentUser?.displayName || 'Administrator'}</p>
