    ...
    return (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            <div className="bg-white w-full max-w-[600px] rounded-3xl shadow-2xl relative flex flex-col animate-in fade-in zoom-in duration-200 overflow-hidden">
                ...
                <div className="p-6 flex gap-4 max-h-[70vh] overflow-y-auto custom-scrollbar">
                    ...
                    <div className="flex-1 flex flex-col">
                        ...
                        <div className="flex items-center gap-4 mt-6 text-gray-400">
                            ...
                            <button onClick={() => fileInputRef.current.click()} className="hover:text-black"><Image size={20} /></button>

                            <div className="relative">
                                <button onClick={() => setShowEmojiPicker(!showEmojiPicker)} className={`transition-colors ${showEmojiPicker ? 'text-sky-500' : 'hover:text-black'}`}><Smile size={20} /></button>
                                {showEmojiPicker && (
                                    <div className="absolute bottom-10 left-0 z-50">
                                        <EmojiPicker onEmojiClick={(e) => setContent(c => c + e.emoji)} theme="light" width={300} height={350} />
                                    </div>
                                )}
                            </div>

                            <div className="relative">
                                <button onClick={() => { if (!showLocationList) fetchLocations(); setShowLocationList(!showLocationList); }} className={`transition-colors ${showLocationList ? 'text-sky-500' : 'hover:text-black'}`}><MapPin size={20} /></button>
                                {showLocationList && (
                                    <div className="absolute bottom-10 left-0 w-64 bg-white shadow-2xl rounded-2xl p-2 border z-50">
                                        <div className="max-h-48 overflow-y-auto">
                                            {locations.map(loc => (
                                                <div key={loc.id} onClick={() => { setSelectedLocation(loc); setShowLocationList(false); }} className="px-3 py-2 hover:bg-gray-50 rounded-xl cursor-pointer text-sm truncate">
                                                    {loc.name}
                                                </div>
                                            ))}
                                        </div>
                            ...
                            <button onClick={() => setShowHashtagInput(!showHashtagInput)} className={`transition-colors ${showHashtagInput ? 'text-sky-500' : 'hover:text-black'}`}><Hash size={20} /></button>
                        </div>
                    </div>
                </div>
            </div>
