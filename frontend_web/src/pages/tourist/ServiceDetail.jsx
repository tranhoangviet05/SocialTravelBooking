...
const ServiceDetail = () => {
    ...
    if (!serviceBase) {
        return <div className="text-center py-20 text-slate-500 font-bold">Không tìm thấy thông tin dịch vụ này.</div>;
    }

    // Gộp dữ liệu hiển thị (Merge)
    const service = {
        name: serviceBase.name,
        provider: serviceBase.provider?.name || 'Social Travel Provider',
        type: serviceBase.type,
        location: serviceBase.location,
        rating: serviceBase.rating,
        reviews: serviceBase.reviewCount,
        price: serviceBase.price,
        oldPrice: serviceBase.originalPrice,
        duration: serviceBase.duration,
        maxGuests: serviceBase.maxGuests,
        images: serviceBase.images || [],
        description: serviceBase.description,
        amenities: serviceBase.highlights || [],
        excludes: ['Chi phí phương tiện di chuyển cá nhân', 'Tiền típ cho HDV (nếu có)', 'Các chi phí ngoài chương trình'],
        itinerary: [
            { day: 1, title: 'Bắt đầu hành trình', content: 'Xe đón quý khách. Check-in nghỉ ngơi và ăn trưa tự túc.' },
            { day: 2, title: 'Trải nghiệm địa phương', content: 'Tham quan theo đoàn và có thời gian tự do khám phá.' },
            { day: 3, title: 'Kết thúc chuyến đi', content: 'Mua sắm đặc sản và làm thủ tục trả phòng, lên xe trở về.' }
        ]
    };
